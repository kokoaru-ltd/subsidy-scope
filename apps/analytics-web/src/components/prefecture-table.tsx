...
import { prefectureTableData } from "@/lib/mock-data";
          ...
          <tbody>
            {prefectureTableData.map((row) => (
              ...
              >
                ...
                <td className="py-3 px-4 text-[var(--text-primary)] font-medium">{row.name}</td>
                <td className="py-3 px-4 text-right text-[var(--text-primary)]">
                  {row.count}
                  <span className="text-[var(--text-dim)] ml-1">件</span>
                </td>
                <td className="py-3 px-4 text-right">
                  <span className="text-[var(--accent)]">{row.activeCount}</span>
                  <span className="text-[var(--text-dim)] ml-1">件</span>
                </td>
                <td className="py-3 px-4 text-right text-[var(--text-secondary)]">{row.totalAmount}</td>
              </tr>
            ))}
          </tbody>
        </table>
